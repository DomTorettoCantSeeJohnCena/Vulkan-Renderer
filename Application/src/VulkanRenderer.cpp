...
struct VulkanData
{
    ...
    std::vector<VkDeviceMemory> DestinationImageMemory;
...
VulkanData* s_VulkanData = new VulkanData(); 
...
void VulkanRenderer::CreateGraphicsPipeline()
{
    ...
    rasterizer.frontFace = VK_FRONT_FACE_CLOCKWISE; // Define the front-facing triangle orientation 
    ...
    pipelineLayoutInfo.setLayoutCount = 0; // Optional: Number of descriptor set layouts 
    pipelineLayoutInfo.pSetLayouts = nullptr; // Optional: Descriptor set layouts 
...
void VulkanRenderer::CreateViewportImage()
{
    s_VulkanData->ViewportImages.resize(s_VulkanData->swapChainImages.size());
    s_VulkanData->DestinationImageMemory.resize(s_VulkanData->swapChainImages.size());
    ...
    for (uint32_t i = 0; i < s_VulkanData->swapChainImages.size(); i++)
    {
        ...
        imageCreateCI.extent.width = s_VulkanData->swapChainExtent.width;
        imageCreateCI.extent.height = s_VulkanData->swapChainExtent.height;
        ...
        // Create the image
        // VkImage dstImage;
        vkCreateImage(s_VulkanData->device, &imageCreateCI, nullptr, &s_VulkanData->ViewportImages[i]);
        ...
        vkAllocateMemory(s_VulkanData->device, &memAllocInfo, nullptr, &s_VulkanData->DestinationImageMemory[i]);
        vkBindImageMemory(s_VulkanData->device, s_VulkanData->ViewportImages[i], s_VulkanData->DestinationImageMemory[i], 0);
...
VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
{
    ...
    if (vkCreateImageView(s_VulkanData->device, &viewInfo, nullptr, &imageView) != VK_SUCCESS)
    {
...
{
    s_VulkanData->ViewportImageViews.resize(s_VulkanData->ViewportImages.size());

    s_VulkanData->ViewportImageViews.resize(s_VulkanData->ViewportImages.size());

    for (uint32_t i = 0; i < s_VulkanData->ViewportImages.size(); i++)
    {
        s_VulkanData->ViewportImageViews[i] = createImageView(s_VulkanData->ViewportImages[i], VK_FORMAT_B8G8R8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
    }
}
    
void VulkanRenderer::ImGuiOnUpdate()
{
